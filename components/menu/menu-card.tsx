...
import { ArrowRight, CircleSlash2, Clock, Heart, Star } from 'lucide-react';
...
const LabelAvailable = ({ item, isShopOpen }: Props) => {
	if (!isShopOpen) {
		return (
	...
	return (
		<div className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 ${className}`}>
			...
			{items.map((item: MenuItem) => {
				const isAvailable = item.stock && isShopOpen;
						...
						<div className="relative aspect-[4/3] overflow-hidden">
							<div className="relative w-full h-full">
								<Image
									...
									alt={item.name}
									fill
									className={`object-cover group-hover:scale-110 transition-transform duration-500 ${!item.stock || !isShopOpen ? 'grayscale' : ''}`}
									loading="lazy"
									onLoad={(e) => {
							...
							<div className="absolute bottom-3 px-6 flex justify-between left-0 right-0 gap-x-2 items-start">
								{/* Name */}
							...
							<div className="flex flex-col h-full">
								...
								<div className="flex items-center justify-between pt-2 mt-5">
									<div className="flex items-center space-x-2">
										<LabelAvailable item={item} isShopOpen={isShopOpen} />
									</div>

									<Link href={`/menu/${item.id}`}>
										<Button size="sm" className="group/btn" disabled={!isAvailable}>
											Detail
